...
@ccclass
export default class QuestionView1 extends cc.Component {
    ...
    public render(data: QuestionViewData): void {
        ...
        const fillBlankChoice = data.fillBlankChoice;
        const optionTexts = fillBlankChoice && fillBlankChoice.options.length > 0
            ? fillBlankChoice.options.map((option) => option.text)
            : null;
        ...
        if (this.quesLabel) {
            this.quesLabel.string = fillBlankChoice ? fillBlankChoice.prompt : data.ques;
        }
    ...
    private resetAnswerState(): void {
        ...
        this.setOutlineColor(this.answerOutlineLabels, QuestionView1.LABEL_OUTLINE_DEFAULT);
    }
    ...
    private applyAnswerVisualState(index: number, state: "default" | "correct" | "wrong"): void {
        ...
        if (sprite) {
            let spriteFrame = QuestionView1.barDefaultFrame;
            if (state === "correct") {
                spriteFrame = QuestionView1.barCorrectFrame || spriteFrame;
            } else if (state === "wrong") {
                spriteFrame = QuestionView1.barWrongFrame || spriteFrame;
            }
            if (spriteFrame) {
    ...
    private getOutlineColorByState(state: "default" | "correct" | "wrong"): cc.Color {
        if (state === "correct") {
            return QuestionView1.LABEL_OUTLINE_CORRECT;
        }
        if (state === "wrong") {
            return QuestionView1.LABEL_OUTLINE_WRONG;
        }
        return QuestionView1.LABEL_OUTLINE_DEFAULT;
    }
    ...
    private preloadAnswerFrames(): void {
        if (QuestionView1.framesLoadingStarted) {
            return;
        }
        QuestionView1.framesLoadingStarted = true;
        ...
        resources.load(QuestionView1.BAR_DEFAULT_PATH, cc.SpriteFrame, (error: Error | null, spriteFrame: cc.SpriteFrame) => {
            if (!error && spriteFrame) {
                QuestionView1.barDefaultFrame = spriteFrame;
                this.resetAnswerState();
            }
        });
        resources.load(QuestionView1.BAR_CORRECT_PATH, cc.SpriteFrame, (error: Error | null, spriteFrame: cc.SpriteFrame) => {
            if (!error && spriteFrame) {
                QuestionView1.barCorrectFrame = spriteFrame;
            }
        });
        resources.load(QuestionView1.BAR_WRONG_PATH, cc.SpriteFrame, (error: Error | null, spriteFrame: cc.SpriteFrame) => {
            if (!error && spriteFrame) {
                QuestionView1.barWrongFrame = spriteFrame;
            }
        });
    }

    private getCorrectAnswerIndex(data: QuestionViewData): number {
        if (!data.fillBlankChoice) {
            return data.correctAnswerIndex ?? 0;
        }

        const optionIndex = data.fillBlankChoice.options.findIndex(
            (option) => option.id === data.fillBlankChoice!.correctOptionId,
        );
        if (optionIndex >= 0) {
            return optionIndex;
        }
    ...
    private getRestPosition(): cc.Vec2 {
        return cc.v2(QuestionView1.DISPLAY_OFFSET_X, QuestionView1.DISPLAY_OFFSET_Y);
    }
}
