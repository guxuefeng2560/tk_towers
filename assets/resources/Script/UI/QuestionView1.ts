...
export default class QuestionView1 extends cc.Component {
    ...
    protected onLoad(): void {
        ...
        this.answerNodes = [this.answer1Node, this.answer2Node];
        this.hideImmediate();
    }
    ...
    private selectAnswer(index: number): void {
        ...
        for (let i = 0; i < this.answerNodes.length; i += 1) {
            this.setNodeColor(this.answerNodes[i], i === index
                ? QuestionView1.COLOR_SELECTED
                : QuestionView1.COLOR_DEFAULT);
        }
        this.submitAnswer();
    }
    ...
    private submitAnswer(): void {
        ...
        if (isCorrect) {
            this.setNodeColor(this.answerNodes[this.selectedIndex], QuestionView1.COLOR_CORRECT);
        } else {
            this.setNodeColor(this.answerNodes[this.selectedIndex], QuestionView1.COLOR_WRONG);
            this.setNodeColor(this.answerNodes[this.correctAnswerIndex], QuestionView1.COLOR_CORRECT);
        }
        playResultStamp(
            this.resultNode,
            this.resultLabel,
            isCorrect,
            QuestionView1.COLOR_CORRECT,
            QuestionView1.COLOR_WRONG,
        );
    ...
    private resetAnswerState(): void {
        for (let i = 0; i < this.answerNodes.length; i += 1) {
            this.setNodeColor(this.answerNodes[i], QuestionView1.COLOR_DEFAULT);
        }
    }
    ...
    private getRestPosition(): cc.Vec2 {
        return cc.v2(QuestionView1.DISPLAY_OFFSET_X, 0);
    }
}
